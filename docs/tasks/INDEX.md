...
| 05  | [Cursor, turret placement, economy](05-cursor-placement-economy.md) | IN PROGRESS | `src/input.*`, `src/game.*`, scenario `build-place-turret` |
| 06  | [Intruder spawning + path movement](06-enemy-spawn-path.md) | TODO | `src/game.*`, `src/level.*`, scenario `enemy-path-march` |
| 07  | [Combat — targeting + damage](07-combat-targeting.md) | TODO | `src/game.*`, scenario `combat-kill` |
| 08  | [Game flow — waves, win/MELTDOWN, HUD, title](08-game-flow-waves.md) | TODO | `src/game.*`, `src/level.*`, scenarios `flow-meltdown`, `flow-win` |
| 09  | [AY sound + content + balance + polish](09-sound-content-polish.md) | TODO | `src/sound.*`, `src/level.*`, scenario `sfx-events` |
